...
export const getCurrentByWalletId = async (idWallet: string) => {
  ...
  const expense = sumAmounts(
    transactions.filter(tx => tx.category?.type === "Expense" || !tx.category?.type)
  );

  // Calculate final balance
  const balance = Number(wallet.initBalance.toNumber()) + income - expense + transferIn - transferOut;
